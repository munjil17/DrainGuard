...
/*
|--------------------------------------------------------------------------
| Issues and affected areas load from database
|--------------------------------------------------------------------------
*/
...
$issueSql = "
    ...
    FROM issues
    ORDER BY issue_name ASC
";

$issueResult = mysqli_query($conn, $issueSql);

if ($issueResult) {
    while ($row = mysqli_fetch_assoc($issueResult)) {
        $issueTypes[] = $row;
    }
}
...
$affectedAreaSql = "
    SELECT affected_area_id, affected_area_name
    FROM affected_areas
    ORDER BY affected_area_name ASC
";

$affectedAreaResult = mysqli_query($conn, $affectedAreaSql);

if ($affectedAreaResult) {
    while ($row = mysqli_fetch_assoc($affectedAreaResult)) {
        $affectedAreas[] = $row;
    }
}
            ...
            >
                ...
                <div class="sc-grid-2">
                    <div class="sc-group">
                        ...
                        <select name="issue_id" id="issueType" required>
                            <option value="">Select issue type</option>

                            <?php foreach ($issueTypes as $issueType): ?>
                                <option value="<?php echo (int)$issueType["issue_id"]; ?>">
                                    <?php echo sc_safe($issueType["issue_name"]); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="sc-group">
                        ...
                        <select name="affected_area_id" id="affectedArea" required>
                            <option value="">Select affected area</option>

                            <?php foreach ($affectedAreas as $affectedArea): ?>
                                <option value="<?php echo (int)$affectedArea["affected_area_id"]; ?>">
                                    <?php echo sc_safe($affectedArea["affected_area_name"]); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
