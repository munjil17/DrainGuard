    ...
    if ($readId > 0 && $userId > 0 && isset($conn) && $conn instanceof mysqli) {
        ...
        if ($readStmt) {
            ...
            if ($readResult && mysqli_num_rows($readResult) === 1) {
                ...
                $updateSql = "UPDATE maintenance_notifications SET is_read = 1 WHERE notification_id = ? AND recipient_user_id = ?";
...
if (isset($_GET["mark_all_read"]) && $_GET["mark_all_read"] === "1") {
    $markAllSql = "UPDATE maintenance_notifications SET is_read = 1 WHERE recipient_user_id = ?";
    ...
    header("Location: /DrainGuard/pages/maintenance/notifications.php");
    exit;
}
...
$whereSql = "mn.recipient_user_id = ?";
$params = [$userId];
$types = "i";

if ($filterType !== "all") {
    $whereSql .= ($whereSql ? " AND " : "") . "mn.notification_type = ?";
    $params[] = $filterType;
    $types .= "s";
}

if ($filterRead === "unread") {
    $whereSql .= ($whereSql ? " AND " : "") . "mn.is_read = 0";
}

if ($filterRead === "read") {
    $whereSql .= ($whereSql ? " AND " : "") . "mn.is_read = 1";
}
...
$countSql = "SELECT COUNT(*) AS total FROM maintenance_notifications mn {$whereSql}";
...
$unreadTotal = 0;
$unreadTotalSql = "SELECT COUNT(*) AS unread_total FROM maintenance_notifications WHERE is_read = 0 AND recipient_user_id = ?";
...
<head>
    <meta charset="UTF-8">
    <title>Notifications | Maintenance Team | DrainGuard</title>
...
<div class="dg-maintenance-layout" style="display: flex; min-height: 100vh;">
        ...
        <section class="nt-page" style="padding: 30px; max-width: 1200px; margin: 0 auto; width: 100%; box-sizing: border-box;">
            ...
            <div class="nt-header">
                <div class="nt-header-left">
                    ...
                    <div>
                        <h1>Notifications</h1>
                        <p>Track your assigned tasks, updates, and alerts</p>
                    </div>
                </div>
...
<script src="../../js/maintenance/notifications.js"></script>
