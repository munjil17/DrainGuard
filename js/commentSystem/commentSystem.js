...
(function () {
    ...
    const complaintId = commentSystem.dataset.complaintId;
    const basePath = commentSystem.dataset.basePath || "../../";

    const list = commentSystem.querySelector("[data-comment-list]");
    const form = commentSystem.querySelector("[data-comment-form]");
    const textarea = commentSystem.querySelector("[data-comment-text]");
    const alertBox = commentSystem.querySelector("[data-comment-alert]");
    const countBox = commentSystem.querySelector("[data-comment-count]");
    ...
    function reactionClass(comment, type) {
        if (comment.my_reaction === type) {
            return type === "like" ? "active-like" : "active-dislike";
        }
        ...
        const replyForm = !isReply
            ? `<form class="dg-reply-form" data-reply-form="${comment.comment_id}">
                    ...
                    <div class="dg-reply-actions">
                        <button type="button" class="dg-reply-cancel" data-reply-cancel="${comment.comment_id}">
                            Cancel
                        </button>

                        <button type="submit" class="dg-reply-submit">
            ...
            <article class="dg-comment-card ${deletedClass}" data-comment-id="${comment.comment_id}">
                    ...
                    <div class="dg-comment-user">
                        ...
                        <div>
                            ...
                            <span>
                                ...
                                ${escapeHtml(comment.user_role_label)}
                            </span>
                        </div>
                    </div>
                ...
                <div class="dg-comment-tools">
                    <button type="button" class="dg-comment-tool ${reactionClass(comment, "like")}" data-react-comment="${comment.comment_id}" data-reaction-type="like">
                        ...
                        <span data-like-count="${comment.comment_id}">${comment.like_count}</span>
                    </button>
                    ...
                    <button type="button" class="dg-comment-tool ${reactionClass(comment, "dislike")}" data-react-comment="${comment.comment_id}" data-reaction-type="dislike">
                        ...
                        <span data-dislike-count="${comment.comment_id}">${comment.dislike_count}</span>
                    </button>
    ...
    async function loadComments() {
        ...
        try {
            const response = await fetch(`${basePath}commentSystem/fetch_comments.php?complaint_id=${encodeURIComponent(complaintId)}`, {
                credentials: "same-origin"
            });
