    ...
    if ($reportType === "ward_complaint_performance") {
        ...
        $sql = "
            SELECT
                ...
                SUM(c.complaint_status IN ('rejected_by_central','rejected_by_ward','final_rejected')) AS rejected_count
            FROM complaints c
    ...
    if ($reportType === "ward_officer_performance") {
        ...
        $sql = "
            SELECT
                ...
                SUM(ca.assignment_status = 'completed') AS completed_count
...
function rowsToTableHtml($title, $rows)
{
    ...
    if (count($rows) === 0) {
        return $html . "<p>No records found for the selected filters.</p>";
    }
...
function writeHtmlFile($path, $title, $rows)
{
    ...
    $html .= "</style>";
    $html .= "    <link rel='stylesheet' href='../css/global/confirm-modal.css'>
</head><body>";
    ...
    $html .= "<script src='../js/global/confirm-modal.js'></script>
</body></html>";
...
function writeSimplePdfFile($path, $title, $rows)
{
    ...
    if ($totalRows === 0) {
        ...
        $content .= "0 0 0 rg\n";
        $content .= pdfText(54, 613, "No records found for the selected filters.", 11);
...
function downloadGeneratedFile($serverPath, $downloadName, $exportFormat)
{
    if (!file_exists($serverPath)) {
        redirectWithError("Unable to create the report. Please try again.");
    }
...
if ($_SERVER["REQUEST_METHOD"] !== "POST") {
    redirectWithError("Invalid request. Please try again. Please try again.");
}
...
if (!in_array($reportType, $validReportTypes, true)) {
    redirectWithError("Please select a valid report type.");
}

if (!in_array($reportPeriod, $validPeriods, true)) {
    redirectWithError("Please select a valid time period.");
}

if (!in_array($exportFormat, $validFormats, true)) {
    redirectWithError("Please select a valid export format.");
}

if ($reportPeriod === "custom_range") {
    if ($customStart === "" || $customEnd === "") {
        redirectWithError("Please select both start and end dates.");
    }

    if ($customStart > $customEnd) {
        redirectWithError("Start date cannot be after the end date.");
    }
}
